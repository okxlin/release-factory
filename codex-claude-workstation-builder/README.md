...
- Before building, it resolves the current proxy-core release inputs, verifies their immutable source archives, selects Go modules compatible with the Dockerfile's Go version, and passes those inputs as build arguments. The resolver keeps gRPC at or below `1.83.2` until the current [CVE-2026-84445](https://github.com/advisories/ghsa-2v4p-qf9q-27wj) branch has a stable patched release. The Dockerfile's proxy pins are local-build fallbacks rather than the CI update source.
