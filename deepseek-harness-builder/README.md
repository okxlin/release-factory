...
For the checked-in source release, Docker verifies the immutable GitHub archive, installs the upstream lockfile, builds the official client and Web UI, packs the DSH and vendor workspaces, and installs those local tarballs into a flat npm runtime tree. Install scripts stay disabled until the reviewed native `koffi`/`node-pty`/`fs-ext` rebuild and the subprocess helper are run explicitly. Published npm selections continue to use the repository's frozen `pnpm-lock.yaml` path. This keeps both release modes reproducible while allowing source-only upstream releases to run through the same patch and smoke contracts.
