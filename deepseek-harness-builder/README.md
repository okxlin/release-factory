...
Pull requests that change the DeepSeek Harness Dockerfile run a read-only component-pin contract check. It rejects floating base-image tags, malformed checksums, and partial updates where repeated Dockerfile version declarations or source URLs no longer agree. This structural check does not publish images or receive registry credentials; the release workflows retain the full build, smoke, and vulnerability gates.
