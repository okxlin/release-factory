...
Release workflows resolve the selected source/npm release once, then use the same [verification action](../.github/actions/verify-deepseek-harness/action.yml). Successful jobs export their tested image with an archive checksum and a receipt containing its config ID, architecture, variant, DSH version, workflow revision, and run ID. The [publication job](../.github/workflows/release-deepseek-harness.yml) checks every archive and loaded image before pushing; it assembles multi-platform tags from the verified registry digests without rebuilding. Both registries must succeed. Version tags are verified in both registries before floating tags are updated; registry writes are not atomic across registries. Release jobs queue instead of cancelling an ongoing publication.

Build caches are scoped by variant and architecture and retain intermediate stages (`mode=max`). Scheduled releases and PR verification refresh the `runtime-base` and, for Workstation, `workstation` APT stages. Manual releases expose `refresh_system_packages` (default: true); local builds provide `--refresh-system-packages`. Source compilation caches remain reusable. Image transfer artifacts expire after one day and raw Trivy reports after seven days. Docker save/load preserves the tested config and layers but does not retain BuildKit provenance attestations; the receipt is workflow evidence, not a signed provenance attestation.

A daily read-only workflow compares the fixed component versions with authoritative GitHub, Go module proxy, Go, Node.js, npm, PyPI, and Python release sources. It writes update candidates to the Actions summary and emits a warning without changing files, opening a pull request, or blocking scheduled releases. Source or policy errors fail the check; a manual run can also enable strict mode to fail when updates are available. A reported version is only a review candidate: checksum and image-digest updates plus the existing build, smoke, and vulnerability gates are still required.

Go is aligned with the official stable version endpoint at <https://go.dev/VERSION?m=text>. Its Docker Official Image index is digest-pinned for reproducible `amd64` and `arm64` selection. actionlint is rebuilt with that pinned Go toolchain from a checksum-pinned official source archive; the other standalone workstation tools are downloaded from their official GitHub releases with architecture-specific SHA-256 checksums.

The three workstation Docker client binaries are rebuilt from checksum-pinned official sources with the pinned Go release. Build and smoke checks inspect each binary's Go package manifest and reject daemon code; the legacy Docker module may contribute only `pkg/namesgenerator`. This avoids maintaining a local Buildx source fork for daemon-only scanner findings. Exact, expiring exceptions and their advisory evidence are recorded in [the image security policy](configs/trivy-policy.json).
...
```bash
# Default/lightweight image. Builds the checked-in source release by default.
...
The local helper uses the checked-in source release and all locked component arguments by default. Use `--version dsh-v<version>` to resolve another source release, or a published npm version/dist-tag for the legacy package path. It prepares source metadata, package-manager versions, and any npm lock changes in a temporary build context. Direct `docker build` calls must supply the arguments emitted by `scripts/component-inputs.py`; use `build-local.sh` to avoid maintaining a separate argument list.
...
The custom Caddy build verifies the Caddy and go-authcrunch source archives, removes go-authcrunch's unused GPG public-key parser, and runs the upstream identity-package tests before linking. It applies the two-line Caddy upstream CEL compatibility fix to the pinned Caddy source, then raises the security-sensitive Go modules to their fixed releases. SSH public-key support remains available to caddy-security, while the generated `CADDY_GO_PACKAGES.txt` manifest must contain no `golang.org/x/crypto/openpgp` package. The rate-limit module and its license are pinned and verified during the same build.
...
`GOMEMLIMIT=128MiB` and `GOMAXPROCS=2` constrain Caddy's Go runtime only. If 1Panel requires a container memory limit, start at `768 MiB` for the lightweight image and at least `1 GiB` for the workstation, then adjust from observed workloads rather than treating the Caddy limit as the whole-container budget.
...
Both architectures use native runners to verify `node-pty`, Caddy modules, authentication, architecture, and the loopback boundary before publication. Workstation runs its compiler and DSH sandbox enforcement probes on amd64 and arm64; CI does not pass `--skip-sandbox-probe`.

## Upgrade behavior

Image scanning uses the explicit Runtime and Workstation profiles in
[`configs/trivy-policy.json`](configs/trivy-policy.json). Both protect DSH and
Caddy HIGH/CRITICAL findings even when no fix is available. Runtime also blocks
other fixable HIGH findings; Workstation reports ordinary toolchain HIGH
findings and blocks fixable CRITICAL findings. Any reviewed exception must name
the exact vulnerability, package version, file path, and profile, with evidence
and an expiry date. See [the security policy](../SECURITY_SCAN.md) for details.
...
Caddy and caddy-security are compiled together and pinned. Updating Caddy alone is not assumed safe. Scheduled release workflows select the highest non-draft GitHub source release, verify its tag, commit, and archive checksum, pass that release as the Docker `DSH_VERSION` build argument, and publish the `runtime` target as `latest` plus `<DSH_VERSION>` and the `workstation` target as `workstation` plus `<DSH_VERSION>-workstation`. Manual runs may select an exact source release, a published DSH npm version/dist-tag, or the final image tag, while retaining the same validation and optional floating tag behavior. Each workflow pushes its verified multi-platform manifest to both GHCR and Docker Hub only after auditing the dependency tree, rebuilding and validating the plugin, running the Caddy dependency-graph/govulncheck gate, and executing its amd64 and arm64 smoke contracts. Runtime releases apply zero-fixable HIGH/CRITICAL Trivy gates. The broader workstation toolchain blocks fixable CRITICAL findings and reports fixable HIGH findings for deterministic review while the daily component check detects new upstream releases. If either registry login or publication fails, the workflow fails instead of reporting a complete release.
