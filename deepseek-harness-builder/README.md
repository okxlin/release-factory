...
Scheduled runs use the checksum-pinned source release recorded in [`image/dsh-source.json`](image/dsh-source.json), then publish matching `<DSH_VERSION>` and `<DSH_VERSION>-workstation` tags. This supports upstream releases that are available from GitHub before they are published to npm; an explicit manual npm version or dist-tag still selects a published package release. Manual workflow runs can also override the published image tag. Use a floating tag for an AppStore `latest` channel and the matching version tag for a numbered AppStore version.

Exact component pins are defined in [the Dockerfile](image/Dockerfile), [package.json](image/package.json), [pnpm-lock.yaml](image/pnpm-lock.yaml), [the checked-in source release](image/dsh-source.json), and the [runtime](../.github/workflows/build-deepseek-harness.yml) and [workstation](../.github/workflows/build-deepseek-harness-workstation.yml) workflows. Those build inputs are the source of truth; this README intentionally describes capabilities and update policy without duplicating version numbers.
...
The local build helper uses the checked-in source release by default and passes its version as Docker `DSH_VERSION`. Use `--version <source-release>` for that release explicitly, or use a published npm version/dist-tag to build the legacy package path. For an npm selector, the helper updates `package.json` and `pnpm-lock.yaml` only in a temporary build context. Direct `docker build` remains supported for the checked-in source baseline.

For the checked-in source release, Docker verifies the immutable GitHub archive, installs the upstream lockfile, builds the official client and Web UI, packs the DSH and vendor workspaces, and installs those local tarballs into a flat npm runtime tree. Install scripts stay disabled until the reviewed native `koffi`/`node-pty` rebuild and the subprocess helper are run explicitly. Published npm selections continue to use the repository's frozen `pnpm-lock.yaml` path. This keeps both release modes reproducible while allowing source-only upstream releases to run through the same patch and smoke contracts.
