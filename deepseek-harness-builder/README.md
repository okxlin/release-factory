...
Exact component pins are defined in [the Dockerfile](image/Dockerfile), [package.json](image/package.json), [pnpm-lock.yaml](image/pnpm-lock.yaml) for the legacy published-npm path, [the checked-in source release](image/dsh-source.json), and the [runtime](../.github/workflows/build-deepseek-harness.yml) and [workstation](../.github/workflows/build-deepseek-harness-workstation.yml) workflows. Source releases use the upstream lockfile while building, then install the locally packed runtime tarballs with npm; the audit checks that resulting npm tree. Those build inputs are the source of truth; this README intentionally describes capabilities and update policy without duplicating version numbers.
...
The local build helper uses the checked-in source release by default and passes its version as Docker `DSH_VERSION`. Use `--version <source-release>` for that release explicitly, or use a published npm version/dist-tag to build the legacy package path. For an npm selector, the helper updates `package.json` and `pnpm-lock.yaml` only in a temporary build context. Direct `docker build` remains supported for the checked-in source baseline; GitHub Actions injects the selected source metadata into the per-run build context, so a new source release does not require a Dockerfile edit.
