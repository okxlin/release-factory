...
# shellcheck source=gemini-skill-browser-linuxserver-builder/configs/architectures.sh
source "$ROOT_DIR/configs/architectures.sh"

IMAGE_REPO="gemini-skill-browser"
DEFAULT_PLATFORM="linux/amd64"
IMAGE_TAG=""
DEFAULT_BROWSER_BASE_TAG="147.0.7727"
BROWSER_BASE_TAG="$DEFAULT_BROWSER_BASE_TAG"
GEMINI_SKILL_REF="main"
PUSH_LATEST="false"
LATEST_TAG="latest-linuxserver"
GITHUB_OUTPUT_PATH=""

usage() {
  cat <<USAGE
用途：解析 Gemini Skill Browser linuxserver 变体的镜像构建参数，供 GitHub Actions 调用。
...
参数：
  ...
  --image-tag <tag>           显式镜像标签；留空时默认跟随浏览器 tag 并追加 -linuxserver
  --gemini-skill-ref <ref>    gemini-skill git ref
  --push-latest <bool>        是否附带别名标签
  --latest-tag <tag>          附带别名标签名称（默认 latest-linuxserver）
...
if [[ -z "$IMAGE_TAG" ]]; then
  IMAGE_TAG="${BROWSER_BASE_TAG}-linuxserver"
else
